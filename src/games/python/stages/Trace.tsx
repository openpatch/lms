...
import type { CodeAnswerQuestion } from "../../../../shared/games/python";
...
/**
 * The body of five stations: read the listing, be the interpreter, write down
 * what comes out. Only the generator behind them differs — output, variables,
 * loops, functions and lists all ask the same question of the player.
 */
  ...
  return (
    <div key={question.id} className="animate-question-in flex w-full max-w-xl flex-col items-center gap-4">
      ...
      <p className="text-center text-gray-500">
        {t(`games.python.ask.${question.ask}`, { name: question.askArg })}
      </p>
      ...
      <input
        ...
        placeholder={t(multiline ? "games.python.answerSeveral" : "games.python.answerOne")}
      ...
      {multiline && (
        <p className="text-center text-xs text-gray-400">
          {t("games.python.answerHint", { count: question.expected.length })}
        </p>
      )}
...
export function TraceRulesExample() {
  return (
    <div className="flex flex-col items-center gap-3">
      <CodeBlock lines={["zahl = 7", "zahl = zahl * 3", "print(zahl)"]} className="max-w-xs" />
