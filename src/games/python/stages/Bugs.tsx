...
import type { BugQuestion } from "../../../../shared/games/python";
 ...
 * is being read — the one place in the framework where a stage can say *why*.
 */
      ...
      {previous && verdict && (
        ...
        >
          <span className="font-bold">
            {verdict.correct ? "✓" : "✗"} {t("games.python.bugs.wasLine", { line: previous.errorLine + 1 })}
          </span>{" "}
          {t(previous.reasonKey)}
        </div>
      )}

      <p className="text-center text-gray-500">{t("games.python.bugsPrompt")}</p>
...
export function BugsRulesExample() {
  return (
    <CodeBlock
      lines={["zahl = 7", "if zahl > 5", '    print("groesser")']}
      numbered
      marked={1}
      className="max-w-xs"
    />
