...
import type { ParsonsQuestion } from "../../../../shared/games/python";
 ...
 */
export default function ParsonsStage({ question, submit }: StageProps<ParsonsQuestion>) {
  ...
  return (
    <div key={question.id} className="animate-question-in flex w-full max-w-xl flex-col gap-4">
      <p className="text-center text-gray-500">
        {t(question.captionKey)} — {t(freeIndent ? "games.python.parsons.withIndent" : "games.python.parsons.plain")}
      </p>

      {/* The program being built */}
      <div className="min-h-[5rem] rounded-xl border-2 border-dashed border-game-200 bg-game-50 p-2">
        {order.length === 0 ? (
          <p className="py-4 text-center text-sm text-gray-400">{t("games.python.parsons.empty")}</p>
        ) : (
            ...
            {order.map((line, position) => (
              ...
              >
                {freeIndent && (
                  <>
                    <IconButton
                      label={t("games.python.parsons.outdent")}
                    ...
                    <IconButton
                      label={t("games.python.parsons.indent")}
                      onClick={() => setIndent(position, 1)}
                ...
                <IconButton label={t("games.python.parsons.up")} onClick={() => move(position, -1)} disabled={position === 0}>
                  ↑
                </IconButton>
                <IconButton
                  label={t("games.python.parsons.down")}
                  onClick={() => move(position, 1)}
                  disabled={position === order.length - 1}
                >
                  ↓
                </IconButton>
                <IconButton label={t("games.python.parsons.back")} onClick={() => remove(position)}>
                  ✕
                </IconButton>
              </li>
            ))}
...
export function ParsonsRulesExample() {
  return (
    <div className="flex flex-col gap-1 text-left font-mono text-sm text-slate-700">
      {["summe = 0", "for zahl in zahlen:", "    summe = summe + zahl", "print(summe)"].map((line, index) => (
        <div key={index} className="rounded-lg border border-gray-200 bg-white px-3 py-1 whitespace-pre">
          <CodeLine text={line} />
        </div>
      ))}
    </div>
